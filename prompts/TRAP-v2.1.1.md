# True Readiness Audit Prompt (TRAP) — v2.1.1
...
**Regex denial of service (ReDoS).** Enumerate the regexes that run against untrusted input and flag any that can backtrack catastrophically — nested or overlapping quantifiers (`(a+)+`, `(.*)*`), or unbounded alternation matched against unbounded input. A single crafted string can pin a CPU core and stall the process; on a single-threaded runtime (e.g. Node) it also freezes every unrelated request in flight. Cite the pattern, its file:line, and the input that reaches it. Recommend a concrete fix — cap the input length, anchor the pattern, or move to a linear-time engine (RE2) — not just "review the regex."
